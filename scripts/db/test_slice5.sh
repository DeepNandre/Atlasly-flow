...
echo "running contract tests: 001, 002, 003, 004, 005"
psql -v ON_ERROR_STOP=1 -h "$SOCK_DIR" -p "$PORT" -d "$DB_NAME" -f "$TESTS_DIR/001_slice1_contracts.sql" >/dev/null
psql -v ON_ERROR_STOP=1 -h "$SOCK_DIR" -p "$PORT" -d "$DB_NAME" -f "$TESTS_DIR/002_slice2_core_domain_contracts.sql" >/dev/null
psql -v ON_ERROR_STOP=1 -h "$SOCK_DIR" -p "$PORT" -d "$DB_NAME" -f "$TESTS_DIR/003_slice3_documents_contracts.sql" >/dev/null
psql -v ON_ERROR_STOP=1 -h "$SOCK_DIR" -p "$PORT" -d "$DB_NAME" -f "$TESTS_DIR/004_slice4_events_contracts.sql" >/dev/null
psql -v ON_ERROR_STOP=1 -h "$SOCK_DIR" -p "$PORT" -d "$DB_NAME" -f "$TESTS_DIR/005_slice5_notifications_contracts.sql" >/dev/null
