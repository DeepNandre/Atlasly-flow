...
export function DemoToolbar({ onRoleChange, currentRole }: DemoToolbarProps) {
  ...
  const run = async (label: string, fn: () => Promise<unknown>) => {
    try {
      ...
      const entry = `[${new Date().toLocaleTimeString()}] ERROR ${label}: ${String(e)}`
      setLog((l) => [entry, ...l].slice(0, 50))
      ...
      <div className="flex items-center gap-2 px-4 py-2">
        ...
        <div className="flex gap-2 flex-1">
          <Button size="sm" variant="ghost" className="text-white h-7 text-xs"
            onClick={() => run('Bootstrap', () => api.post('/api/bootstrap', { org_name: 'Demo Org', user_name: 'owner', email: 'owner@demo.com' }))}>
            Bootstrap
          </Button>
          <Button size="sm" variant="ghost" className="text-white h-7 text-xs"
            onClick={() => run('Demo Scenario', () => api.post('/api/demo/run-scenario'))}>
            Run Demo
          </Button>
          <Button size="sm" variant="ghost" className="text-white h-7 text-xs"
            onClick={() => run('Reset', () => api.post('/api/demo/reset'))}>
            Reset
          </Button>
          <div className="w-36">
            <Select value={currentRole} onValueChange={(v) => onRoleChange(v as Role)}>
              <SelectTrigger className="h-7 text-xs bg-atlasly-slate border-atlasly-line text-white">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {(['owner', 'admin', 'pm', 'reviewer', 'subcontractor'] as Role[]).map((r) => (
        ...
        <button onClick={() => setOpen((o) => !o)} className="ml-auto opacity-60 hover:opacity-100">
          {open ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronUp className="h-3.5 w-3.5" />}
        </button>
      </div>
      {open && (
        <div className="h-36 overflow-y-auto px-4 pb-2 space-y-0.5 border-t border-atlasly-line/30">
          {log.length === 0 && <p className="text-atlasly-muted py-2">No log entries yet</p>}
          {log.map((entry, i) => (
            <p key={i} className="text-atlasly-line/80 leading-relaxed">{entry}</p>
          ))}
        </div>
      )}
    </div>
