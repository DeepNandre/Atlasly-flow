...
apply_sql "$MIGRATIONS_DIR/000032_stage3_foundations.sql"
apply_sql "$MIGRATIONS_DIR/000033_stage3_persistence_scaffolding.sql"

echo "running stage3 contract tests: 006"
psql -v ON_ERROR_STOP=1 -h "$SOCK_DIR" -p "$PORT" -d "$DB_NAME" -f "$TESTS_DIR/006_stage3_persistence_contracts.sql" >/dev/null

echo "verifying rollback migrations"
apply_sql "$ROLLBACK_DIR/000033_stage3_persistence_scaffolding_rollback.sql"
apply_sql "$ROLLBACK_DIR/000032_stage3_foundations_rollback.sql"
...
echo "stage3 slice6 tests passed"
