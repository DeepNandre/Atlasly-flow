...
apply_sql "$MIGRATIONS_DIR/000009_stage0_future_moat_tables.up.sql"

echo "running contract tests: 001..007"
for test in \
  ...
  006_slice6_rls_contracts.sql \
  007_slice7_moat_contracts.sql; do
  psql -v ON_ERROR_STOP=1 -h "$SOCK_DIR" -p "$PORT" -d "$DB_NAME" -f "$TESTS_DIR/$test" >/dev/null
done

echo "verifying down migrations"
apply_sql "$MIGRATIONS_DIR/000009_stage0_future_moat_tables.down.sql"
