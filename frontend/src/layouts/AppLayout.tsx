    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? 'bg-atlasly-teal/15 text-atlasly-teal'
            : 'text-atlasly-muted hover:bg-atlasly-bg hover:text-atlasly-ink',
        )
      }
    >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
        <div>
          ...
          <div className="space-y-0.5">
            {NAV_WORK.map((item) => {
              if (item.to === '/payouts' && !perms.canViewPayouts) return null
          ...
          <div className="absolute bottom-full left-3 right-3 mb-1 rounded-md border border-atlasly-line bg-atlasly-paper shadow-lg py-1 z-50">
            ...
            {(['owner', 'admin', 'pm', 'reviewer', 'subcontractor'] as Role[]).map((r) => (
              <button
                ...
                className={cn(
                  'flex items-center gap-2 w-full px-3 py-1.5 text-sm hover:bg-atlasly-bg',
                  r === role ? 'text-atlasly-teal font-medium' : 'text-atlasly-ink',
                )}
              >
                {ROLE_LABELS[r]}
    ...
    <div className="flex h-screen overflow-hidden bg-atlasly-bg">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          <div className="absolute inset-0 bg-atlasly-ink/50" onClick={() => setMobileOpen(false)} />
      ...
      {mobileOpen && (
        <button
          className="fixed top-4 left-64 z-50 md:hidden text-white"
      ...
      {isDemo && <DemoToolbar onRoleChange={switchRole} currentRole={role} />}
    </div>
  )
}
